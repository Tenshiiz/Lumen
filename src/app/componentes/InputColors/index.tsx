...
interface InputColorsProps {
  value: string; // valor HEX vindo do pai
  onChange: (value: string) => void; // atualiza o estado no pai
  label?: string;
  width?: string;
...
function InputColors({
  ...
  useEffect(() => {
    ...
    if (format === 'rgb') {
      setDisplayValue(`${rgb.r}, ${rgb.g}, ${rgb.b}`);
    } else if (format === 'hsl') {
      ...
      setDisplayValue(value.toUpperCase());
    }
  }, [value, format]);
  ...
  const handleHexChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    hex = `#${hex}`;
    setDisplayValue(hex.toUpperCase());

    // só dispara o onChange se tiver 6 caracteres válidos
    if (hex.length === 7) {
    ...
    <div className="flex flex-col gap-1">
      ...
      {format === 'hex' ? (
        <input
          ...
          maxLength={6}
          className={`border border-gray-700 rounded-md p-2 ${width} bg-[#191c1f] text-sm text-center text-white font-mono`}
          placeholder="000000"
        />
      ) : (
        <input
